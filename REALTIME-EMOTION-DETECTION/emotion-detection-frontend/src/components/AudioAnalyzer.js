const convertBlobToWav = async (audioBlob) => {
  ...
  const audioBuffer = await audioContext.decodeAudioData(arrayBuffer); // Decode the original Blob to PCM

  return encodeWAV(audioBuffer); // Convert to proper WAV format
};

// Function to convert decoded PCM data into WAV format
